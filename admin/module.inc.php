...
$options=array(-1=>'安装包损坏',-2=>'安装包解压失败',-3=>'重复安装',-4=>'安装包损坏',-5=>'安装包解压失败',-6=>'安装包解压失败');
...

{
	...
	case 'diy':
		...
		break;
	case 'set':
		$data=Module::getModuleByKey($modulekey);

		$data['configmenu']=json_decode($data['configmenu'],true);
		...
		if($dosubmit)
		{
			...
			$info['ispch5']=intval($info['ispch5']);

			$info['configmenu']=urldecode(json_encode($configmenu));

			$info['folder']=$data['folder'];
			$info['key']=pw_md5(Module::getModuleByKey($info['parentkey'],'folder').'/'.$info['folder'].'/');

			Module::moduleSet($_POST['key'],$info);
			operation_tips('模块配置成功！','?mod=&file=module&action=manage&parentkey='.$info['parentkey']);
		}
		include_once parse_admin_tlp($file.'-'.$action);
		break;
	case 'tables':
	...
	case 'import':
...

		{
...

			{
				...
				operation_tips('操作失败 ['.$options[$op].']！','','error');
